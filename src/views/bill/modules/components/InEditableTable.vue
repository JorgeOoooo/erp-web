    ...
    <a-form-model ref="formRef" :model="form" :validateOnRuleChange="true">
      ...
      >
        ...
        >
          ...
          >
            <template v-if="col.dataIndex == 'standard'">
              <a-input
                v-if="record?.['SHOW_STATUS_model'] == 'warning'"
            ...
            <template v-if="col.dataIndex == 'pricingType'">
              <a-select
                v-if="record?.['SHOW_STATUS_model'] == 'warning'"
                v-model="record[col.dataIndex]"
            ...
            <a-select
              v-else-if="col.type == FormTypes.lazySelect"
              v-model="record[col.dataIndex]"
              :dropdownMatchSelectWidth="false"
        ...
        <template slot="action" slot-scope="text, record, index">
          ...
          <a-tooltip
            v-if="
              index + 1 != form.dataSource.length &&
              record?.['SHOW_STATUS_model'] == 'warning'
            "
            placement="top"
          >
            <template slot="title">
              <span>将该条目入库，并刷新列表。</span>
            </template>
            <a-button
              @click="handleAddMaterial(record, index)"
              type="link"
              size="small"
              :tabindex="-1"
            >
...
export default {
  ...
  methods: {
    ...
    handleAddMaterial(record, index) {
      if (!record?.model) {
        this.$message.error("请填写款号");
        return;
      }
      if (!record?.pricingType) {
        this.$message.error("请填写计价类型");
        return;
      }
      const params = {
        model: record.model,
      ...
      const that = this;
      httpAction("/material/add", params, "post")
        .then((res) => {
          if (res.code === 200) {
            this.$message.success("添加成功");
            this.form.dataSource.forEach((item) => {
              if (item.model == record.model) {
                this.$set(item, "SHOW_model", {
                  standard: record?.standard,
                  pricingType: record?.pricingType,
                });
                this.$set(item, "SHOW_STATUS_model", "success");
              }
            });
          } else {
            that.$message.warning(res.data.message || res.data);
          }
        })
        .finally(() => {
