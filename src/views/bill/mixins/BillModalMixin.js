...
export const BillModalMixin = {
  ...
  methods: {
    ...
    selectAccount(value) {
      if (value === 0) {
        //多账户
        this.$refs.manyAccountModalForm.edit(
          this.accountIdList,
          this.accountMoneyList
        );
    ...
    manyAccountModalFormOk(idList, moneyList, allPrice) {
      ...
      let otherMoney = this.form.getFieldValue("otherMoney")
        ? this.form.getFieldValue("otherMoney") - 0
        : 0;
    ...
    onAdded(event) {
      ...
      if (this.currentSelectDepotId) {
        ...
        target.setValues([
          { rowKey: row.id, values: { depotId: this.currentSelectDepotId } },
        ]);
      } else {
        getAction("/depot/findDepotByCurrentUser").then((res) => {
          if (res.code === 200) {
      ...
      switch (column.key) {
          ...
          getMaterialByBarCode(param).then((res) => {
            if (res && res.code === 200) {
              ...
              if (value.indexOf(",") > -1) {
                ...
                //单个条码
                let depotIdSelected =
                  this.prefixNo !== "CGDD" && this.prefixNo !== "XSDD"
                    ? row.depotId
                    : "";
                findStockByDepotAndBarCode({
        ...
        case "taxRate":
          ...
          target.setValues([
            {
              rowKey: row.id,
              values: { taxMoney: taxMoney, taxLastMoney: taxLastMoney },
            },
          ]);
          target.recalcAllStatisticsColumns();
          that.autoChangePrice(target);
        ...
        case "taxLastMoney":
          ...
          taxRate = row.taxRate - 0; //税率
          unitPrice =
            (taxLastMoney / operNumber / (1 + taxRate * 0.01)).toFixed(2) - 0;
          allPrice = (unitPrice * operNumber).toFixed(2) - 0;
    ...
    getStockByDepotBarCode(row, target) {
      findStockByDepotAndBarCode({
        ...
        if (res && res.code === 200) {
          target.setValues([
            { rowKey: row.id, values: { stock: res.data.stock } },
          ]);
          target.recalcAllStatisticsColumns();
        }
      });
    ...
    onChangeDiscount(e) {
      ...
      let deposit = this.form.getFieldValue("deposit");
      let allTaxLastMoney =
        this.$refs.materialDataTable.statisticsColumns.taxLastMoney - 0;
      let discountMoneyNew = (allTaxLastMoney * value * 0.01).toFixed(2) - 0;
      let discountLastMoneyNew =
        (allTaxLastMoney - discountMoneyNew).toFixed(2) - 0;
            ...
            getMaterialByBarCode(param).then((res) => {
              if (res && res.code === 200) {
                  ...
                  if (detail.barCode) {
                    ...
                    for (let i = 0; i < mList.length; i++) {
                      if (detail.barCode === mList[i].mBarCode) {
                        ...
                        detail.allPrice =
                          (detail.unitPrice * detail.operNumber).toFixed(2) - 0;
                        detail.taxMoney =
                          (taxRate * 0.01 * detail.allPrice).toFixed(2) - 0;
                        detail.taxLastMoney =
                          (detail.allPrice + detail.taxMoney).toFixed(2) - 0;
                      }
                    }
                ...
                for (let newDetail of newDetailArr) {
                  ...
                  allTaxLastMoney =
                    allTaxLastMoney + (newDetail.taxLastMoney - 0);
                }
                let discount = this.form.getFieldValue("discount") - 0;
                let otherMoney = this.form.getFieldValue("otherMoney")
                  ? this.form.getFieldValue("otherMoney") - 0
                ...
                let deposit = this.form.getFieldValue("deposit");
                let discountMoney =
                  (discount * 0.01 * allTaxLastMoney).toFixed(2) - 0;
                let discountLastMoney =
                  (allTaxLastMoney - discountMoney).toFixed(2) - 0;
                let changeAmountNew =
                  (discountLastMoney + otherMoney).toFixed(2) - 0;
                if (deposit) {
                  changeAmountNew = (changeAmountNew - deposit).toFixed(2) - 0;
                }
                this.$nextTick(() => {
                  changeAmountNew =
                    this.prefixNo === "XSDD" ? 0 : changeAmountNew;
                  this.form.setFieldsValue({
                    discount: discount,
                    discountMoney: discountMoney,
                    discountLastMoney: discountLastMoney,
                ...
                for (let detail of detailArr) {
                  if (detail.barCode) {
                    ...
                    if (detail.barCode === this.scanBarCode) {
                      ...
                      detail.allPrice =
                        (unitPrice * detail.operNumber).toFixed(2) - 0;
                      detail.taxMoney =
                        (taxRate * 0.01 * detail.allPrice).toFixed(2) - 0;
                      detail.taxLastMoney =
                        (detail.allPrice + detail.taxMoney).toFixed(2) - 0;
    ...
    handleWorkflow() {
      if (this.model && this.model.number) {
        getPlatformConfigByKey({ platformKey: "send_workflow_url" }).then(
          (res) => {
            if (res && res.code === 200) {
              let sendWorkflowUrl =
                res.data.platformValue + "?no=" + this.model.number + "&type=1";
