...
export default {
  ...
  methods: {
    edit(record) {
      ...
      this.$nextTick(() => {
        this.form.setFieldsValue(
          pick(this.model, "name", "type", "description")
        );
      });
    ...
    handleOk() {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          let obj;
          checkUserBusiness({ type: "UserDepot", keyId: this.roleId }).then(
            (res) => {
    ...
    loadTree(id) {
      ...
      let params = {};
      params.id = "";
      getAction("/depot/findUserDepot?UBType=UserDepot&UBKeyId=" + id).then(
        (res) => {
          if (res) {
            //机构全选后，再添加机构，选中数量增多
            this.allTreeKeys = [];
