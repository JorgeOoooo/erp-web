...
export default {
  ...
  methods: {
    ...
    init() {
        ...
        if (res.code === 200) {
          ...
          if (record.id) {
            ...
            if (record.customerFlag != null) {
              this.customerFlagSwitch =
                record.customerFlag == "1" ? true : false;
            }
            if (record.minusStockFlag != null) {
              this.minusStockFlagSwitch =
                record.minusStockFlag == "1" ? true : false;
            }
            if (record.purchaseBySaleFlag != null) {
              this.purchaseBySaleFlagSwitch =
                record.purchaseBySaleFlag == "1" ? true : false;
            }
            if (record.overLinkBillFlag != null) {
              this.overLinkBillFlagSwitch =
                record.overLinkBillFlag == "1" ? true : false;
            }
            if (record.updateUnitPriceFlag != null) {
              this.updateUnitPriceFlagSwitch =
                record.updateUnitPriceFlag == "1" ? true : false;
            }
            if (record.forceApprovalFlag != null) {
              this.forceApprovalFlagSwitch =
                record.forceApprovalFlag == "1" ? true : false;
            }
            if (record.multiLevelApprovalFlag != null) {
              this.multiLevelApprovalFlagSwitch =
                record.multiLevelApprovalFlag == "1" ? true : false;
              this.originalMultiLevelApprovalFlag =
                record.multiLevelApprovalFlag;
            }
      ...
      //校验是否存在多级审批插件
      getAction("/plugin/checkByPluginId", { pluginIds: "workflow" }).then(
        (res) => {
          if (res.code === 200) {
            if (res.data) {
      ...
      //校验是否存在盘点插件
      getAction("/plugin/checkByPluginId", { pluginIds: "stock-check" }).then(
        (res) => {
          if (res.code === 200) {
            if (res.data) {
              ...
              //校验是否存在生产插件
              getAction("/plugin/checkByPluginId", {
                pluginIds: "produce",
              }).then((res) => {
                if (res.code === 200) {
                  if (res.data) {
          ...
          obj
            .then((res) => {
              if (res.code === 200) {
                ...
                if (
                  this.originalMultiLevelApprovalFlag !=
                    formData.multiLevelApprovalFlag ||
