      ...
      getAction("/sequence/buildNumber").then((res) => {
        if (res && res.code === 200) {
          this.form.setFieldsValue({
            billNo: amountNum + res.data.defaultNumber,
          });
        }
      });
      this.$nextTick(() => {
      ...
      if (organId) {
        this.$confirm({
          ...
          onOk: function () {
            ...
            getAction("/supplier/getBeginNeedByOrganId", {
              organId: organId,
            }).then((res) => {
              if (res.code === 200) {
      ...
      if (this.model && this.model.billNo) {
        getPlatformConfigByKey({ platformKey: "send_workflow_url" }).then(
          (res) => {
            if (res && res.code === 200) {
              let sendWorkflowUrl =
                res.data.platformValue + "?no=" + this.model.billNo + "&type=2";
