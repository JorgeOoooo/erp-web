      ...
      <a-form :form="form">
        <a-tabs default-active-key="1" size="small">
          <a-tab-pane key="1" tab="基本信息" id="materialHeadModal" forceRender>
              ...
              <a-col :md="6" :sm="24">
                ...
                >
                  <a-row class="form-row" :gutter="24">
                    ...
                    >
                      <a-checkbox :checked="unitChecked" @change="unitOnChange"
                        >多单位</a-checkbox
                      >
                    </a-col>
                  </a-row>
            ...
            <a-row class="form-row" :gutter="24">
              <a-col :md="6" :sm="24">
                ...
                >
                  <a-tooltip
                    title="如果选择为有，则在采购入库单需要录入该商品的序列号"
                  >
                    <a-select
                      placeholder="有无序列号"
                      v-decorator="['enableSerialNumber']"
                    >
              ...
              <a-col :md="6" :sm="24">
                ...
                >
                  <a-tooltip
                    title="如果选择为有，则在采购入库单需要录入该商品的批号和有效期"
                  >
                    <a-select
                      placeholder="有无批号"
                      v-decorator="['enableBatchNumber']"
                    >
                      <a-select-option value="1">有</a-select-option>
                      <a-select-option value="0">无</a-select-option>
                    </a-select>
              ...
              <a-col :md="6" :sm="24" v-if="!model.id">
                ...
                >
                  <a-tooltip
                    title="多属性针对服装、鞋帽等行业，需要先录入单位才能激活此处输入框"
                  >
                    <a-tag class="tag-info" v-if="!manySkuStatus"
                      >需要先录入单位才能激活</a-tag
                    >
                    <a-select
                      mode="multiple"
            ...
            <div style="margin-top: 8px" id="materialDetailModal">
              ...
              >
                <template #buttonAfter>
                  <a-button @click="batchSetPrice('purchase')"
                    >采购价-批量</a-button
                  >
                  <a-button
                    style="margin-left: 8px"
                    @click="batchSetPrice('commodity')"
                    >零售价-批量</a-button
                  >
                  <a-button
                    style="margin-left: 8px"
                    @click="batchSetPrice('wholesale')"
                    >销售价-批量</a-button
                  >
                  <a-button
                    style="margin-left: 8px"
                    @click="batchSetPrice('low')"
                    >最低售价-批量</a-button
                  >
                </template>
              </j-editable-table>
          ...
          <a-tab-pane key="2" tab="扩展信息" forceRender>
            ...
            <a-row
              v-if="mpShort.otherField1.enabled"
              class="form-row"
              :gutter="24"
            >
              <a-col :lg="6" :md="6" :sm="6">
                <a-form-item
                  :labelCol="labelCol"
                  :wrapperCol="wrapperCol"
                  :label="mpShort.otherField1.name"
                >
            ...
            <a-row
              v-if="mpShort.otherField2.enabled"
              class="form-row"
              :gutter="24"
            >
              <a-col :lg="6" :md="6" :sm="6">
                <a-form-item
                  :labelCol="labelCol"
                  :wrapperCol="wrapperCol"
                  :label="mpShort.otherField2.name"
                >
            ...
            <a-row
              v-if="mpShort.otherField3.enabled"
              class="form-row"
              :gutter="24"
            >
              <a-col :lg="6" :md="6" :sm="6">
                <a-form-item
                  :labelCol="labelCol"
                  :wrapperCol="wrapperCol"
                  :label="mpShort.otherField3.name"
                >
            ...
            >
              <template #buttonAfter>
                <a-button
                  ...
                  @click="batchSetStock('initStock')"
                  >期初库存-批量</a-button
                >
                <a-button
                  style="margin-left: 8px"
                  @click="batchSetStock('lowSafeStock')"
                  >最低安全库存-批量</a-button
                >
                <a-button
                  style="margin-left: 8px"
                  @click="batchSetStock('highSafeStock')"
                  >最高安全库存-批量</a-button
                >
              </template>
            </j-editable-table>
...
export default {
  ...
  methods: {
    ...
    edit(record) {
      ...
      if (this.model.id) {
        ...
        this.requestMeTableData(
          this.url.materialsExtendList,
          params,
          this.meTable
        );
        this.requestDepotTableData(
          this.url.depotWithStock,
          { mId: this.model.id },
          this.depotTable
        );
      } else {
    ...
    requestAddOrEdit(formData) {
      ...
      if (
        formData.enableSerialNumber === "1" &&
        formData.enableBatchNumber === "1"
      ) {
        ...
        if (res && res.code === 200) {
          if (res.data.status) {
            ...
            for (let i = 0; i < formData.stock.length; i++) {
              ...
              if (
                skuCount &&
                depotStockObj.initStock &&
                depotStockObj.initStock - 0
              ) {
                this.$message.warning(
                  "抱歉，多属性商品不能录入期初库存，建议进行盘点录入！"
                );
                return;
              }
              if (depotStockObj.lowSafeStock && depotStockObj.highSafeStock) {
                if (
                  depotStockObj.lowSafeStock - 0 >
                  depotStockObj.highSafeStock - 0
                ) {
                  this.$message.warning(
                    "抱歉，" +
                      depotStockObj.name +
    ...
    onManySkuChange(value) {
      ...
      if (value.length < 3) {
        this.materialAttributeList.forEach((item, index, array) => {
          array.indexOf(item.value) === -1
            ? Vue.set(array[index], "disabled", false)
            : "";
        });
      } else {
        this.materialAttributeList.forEach((item, index, array) => {
          value.indexOf(item.value) === -1
            ? Vue.set(array[index], "disabled", true)
            : "";
        });
      }
      //更新属性1和属性2和属性3的下拉框
    ...
    autoSkuList(skuOneData, skuTwoData, skuThreeData) {
      ...
      if (unit) {
        ...
        let count =
          this.getNumByField("skuOne") +
          this.getNumByField("skuTwo") +
          this.getNumByField("skuThree");
        let barCodeSku = [];
        if (count === 1) {
          let skuArrOnly = [];
          if (this.getNumByField("skuOne")) {
          ...
          for (let i = 0; i < skuArrOne.length; i++) {
            for (let j = 0; j < skuArrTwo.length; j++) {
              for (let k = 0; k < skuArrThree.length; k++) {
                barCodeSku.push(
                  skuArrOne[i] + "/" + skuArrTwo[j] + "/" + skuArrThree[k]
                );
              }
            }
          }
          ...
          for (let i = 0; i < mArr.length; i++) {
            ...
            if (i === 0) {
              ...
              if (basicPurchaseDecimal) {
                mInfo.purchaseDecimal = (basicPurchaseDecimal * ratio).toFixed(
                  2
                );
              }
              if (basicCommodityDecimal) {
                mInfo.commodityDecimal = (
                  basicCommodityDecimal * ratio
                ).toFixed(2);
              }
              if (basicWholesaleDecimal) {
                mInfo.wholesaleDecimal = (
                  basicWholesaleDecimal * ratio
                ).toFixed(2);
              }
              if (basicLowDecimal) {
