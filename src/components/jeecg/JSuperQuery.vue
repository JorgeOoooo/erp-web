<template>
  <div class="j-super-query-box">
    ...
    >
      ...
      <a-button v-else type="primary" icon="filter" @click="handleOpen"
        >高级查询</a-button
      >
    </slot>
    ...
    >
      ...
      <a-spin :spinning="loading">
        <a-row>
          <a-col :sm="24" :md="24 - 5">
            <a-empty
              v-if="queryParamsModel.length === 0"
            ...
            <a-form v-else layout="inline">
                  ...
                  >
                    ...
                    >
                      <a-select-option value="and"
                        >AND（所有条件都要求匹配）</a-select-option
                      >
                      <a-select-option value="or"
                        >OR（条件中的任意一个匹配）</a-select-option
                      >
                    </a-select>
                  </a-form-item>
                </a-col>
              ...
              >
                ...
                <a-col :md="4" :xs="24" style="margin-bottom: 12px">
                  ...
                  >
                    ...
                    <a-select-option value="right_like"
                      >以..开始</a-select-option
                    >
                    <a-select-option value="left_like"
                      >以..结尾</a-select-option
                    >
                ...
                <a-col :md="8" :xs="24" style="margin-bottom: 12px">
                  ...
                  <j-select-multi-user
                    v-else-if="
                      item.type === 'select-user' || item.type === 'sel_user'
                    "
                    v-model="item.val"
                    :buttons="false"
                    :multiple="false"
                  ...
                  <j-area-linkage
                    v-model="item.val"
                    v-else-if="
                      item.type === 'area-linkage' || item.type === 'pca'
                    "
                    style="width: 100%"
                  />
                  <j-date
...
export default {
  ...
  methods: {
    ...
    handleSelected(node, item) {
      let { type, options, dictCode, dictTable, customReturnField, popup } =
        node.dataRef;
      item["type"] = type;
    ...
    // 将查询保存到 LocalStore 里
    saveToLocalStore() {
      let saveValue = this.saveTreeData.map(
        ({ originTitle, matchType, records }) => ({
          title: originTitle,
          matchType,
          records,
        })
